...
While comparing the mean weight of Pacific giant salamander is useful, there are some concerns about the data that suggest that medians may be a more useful measure of comparison. For one, the data have a heavy positive skew and are potentially non-normal. Outliers may also be heavily influencing the mean. (For instance, the mean weight of salamders found in pools is `r round(salamander_weight_summary$mean_weight[2],2)` g, while the median weight is `r round(salamander_weight_summary$median_weight[2],2)` g.) A Levene's test indicates that the variances are equal, and ANOVAs are relatively resilient to violations of normality, so comparing means could wbe appropriate. However, given this non-normal data, a non-parametric rank based test that compares medians (such as Kruskal-Wallis) may be more approprirate for use.
